namespace NovaWright.NumberPush.LevelGenerator
{
    public class NumberPushGenerationDiagnostics
    {
        ...
        public Dictionary<int, int> SolvableCandidatePushCounts { get; set; } = new();

        public Dictionary<int, int> UnsolvableCandidateMaximumPushCounts { get; set; } = new();

        public Dictionary<int, int> InitialCrateMobilityDistribution { get; set; } = new();

        public Dictionary<int, int> InitialPlayerPushMobilityDistribution { get; set; } = new();
        ...
        public Dictionary<int, Point> CrateStartPositions { get; set; } = new();

        public Dictionary<int, int> CrateDistances { get; set; } = new();

        public Dictionary<int, HashSet<Point>> CrateReachableGoals { get; set; } = new();

        public Dictionary<int, HashSet<Point>> CrateReachablePositions { get; set; } = new();

        public Dictionary<int, Dictionary<Point, int>> CrateGoalPushDistances { get; set; } = new();
        ...
        public double AverageSolverMilliseconds
        {
            get
            {
                ...
                return (double)SolverMilliseconds / TotalSolverCalls;
            }
        }
        ...
        public string GetReportText()
        {
            ...
            foreach (int pushCount in SolvableCandidatePushCounts.Keys.OrderBy(pushes => pushes))
            {
                report +=
                    $"  {pushCount} pushes: " + $"{SolvableCandidatePushCounts[pushCount]}\r\n";
            }

            report += "\r\n" + "UNSOLVABLE CANDIDATE MAXIMUM PUSH DISTRIBUTION\r\n";

            foreach (
                int pushCount in UnsolvableCandidateMaximumPushCounts.Keys.OrderBy(pushes => pushes)
            )
            {
                report +=
            ...
            foreach (
                int depth in UnsolvableCandidateMinimumZeroPushDepths.Keys.OrderBy(value => value)
            )
            {
                report +=
                    $"  Depth {depth}: " + $"{UnsolvableCandidateMinimumZeroPushDepths[depth]}\r\n";
            }

            report += "\r\n" + "UNSOLVABLE CANDIDATE LEGAL PUSHES BEFORE FIRST ZERO-PUSH STATE\r\n";

            foreach (
                int legalPushes in UnsolvableCandidateLegalPushesBeforeZero.Keys.OrderBy(value =>
                    value
                )
            )
            {
            ...
            foreach (int crateIndex in CratePushCounts.Keys.OrderBy(index => index))
            {
                int crateNumber = crateIndex + 1;
            ...
            foreach (int mobility in InitialCrateMobilityDistribution.Keys.OrderBy(value => value))
            {
                report +=
            ...
            foreach (
                int mobility in InitialPlayerPushMobilityDistribution.Keys.OrderBy(value => value)
            )
            {
                report +=
                    $"  {mobility} legal pushes: "
            ...
            foreach (
                int mobility in InitialPlayerMobilityMaximumPushDistribution.Keys.OrderBy(value =>
                    value
                )
            )
            {
                report += $"  Initial {mobility} legal pushes:\r\n";

                foreach (
                    int maximumPushes in InitialPlayerMobilityMaximumPushDistribution[mobility]
            ...
            foreach (
                int initialMobility in InitialToAfterFirstPushMobilityDistribution.Keys.OrderBy(
                    value => value
                )
            )
            {
                report += $"  Initial {initialMobility} legal pushes:\r\n";

                Dictionary<int, int> afterFirstPushDistribution =
                    InitialToAfterFirstPushMobilityDistribution[initialMobility];

                foreach (
                    int afterFirstPushMobility in afterFirstPushDistribution.Keys.OrderBy(value =>
                        value
                    )
                )
                {
            ...
            foreach (int crateIndex in CrateStartPositions.Keys.OrderBy(index => index))
            {
                int crateNumber = crateIndex + 1;

                Point startPosition = CrateStartPositions[crateIndex];

                int distance = CrateDistances[crateIndex];

                HashSet<Point> reachableGoals = CrateReachableGoals[crateIndex];

                Dictionary<Point, int> goalPushDistances = CrateGoalPushDistances[crateIndex];

                HashSet<Point> reachablePositions = CrateReachablePositions[crateIndex];
                ...
                report += $"  Reachable positions: " + $"{reachablePositions.Count}\r\n";

                List<Point> orderedReachablePositions = reachablePositions
                ...
                {
                    List<string> positions = orderedReachablePositions
                        .Skip(positionIndex)
                        .Take(15)
                        .Select(point => $"({point.X},{point.Y})")
                        .ToList();

                    report += "    " + string.Join(" ", positions) + "\r\n";
                }

                foreach (
                    Point goal in reachableGoals.OrderBy(point => point.Y).ThenBy(point => point.X)
                )
                {
                    report +=
                        $"  Goal: ({goal.X},{goal.Y}), "
            ...
            foreach (int crateIndex in CrateGoalPositions.Keys.OrderBy(index => index))
            {
                int crateNumber = crateIndex + 1;

                Point goalPosition = CrateGoalPositions[crateIndex];

                report +=
                    $"Crate {crateNumber} goal: " + $"({goalPosition.X},{goalPosition.Y})\r\n";
            }
            ...
            foreach (int crateIndex in SolutionPlayerAccessBlocks.Keys.OrderBy(index => index))
            {
                int crateNumber = crateIndex + 1;

                report += $"Crate {crateNumber} blocks player access to: ";

                List<int> affectedCrates = SolutionPlayerAccessBlocks[crateIndex]
                    .OrderBy(index => index)
                    .Select(index => index + 1)
                    .ToList();

                report += string.Join(", ", affectedCrates) + "\r\n";
            }
            ...
            foreach (int crateIndex in SolutionRequiredDependencies.Keys.OrderBy(index => index))
            {
                int crateNumber = crateIndex + 1;

                report += $"Crate {crateNumber} must move before: ";

                List<int> affectedCrates = SolutionRequiredDependencies[crateIndex]
                    .OrderBy(index => index)
                    .Select(index => index + 1)
                    .ToList();
            ...
            foreach (int crateIndex in SolutionOpportunities.Keys.OrderBy(index => index))
            {
                int crateNumber = crateIndex + 1;

                report += $"Crate {crateNumber} opens opportunities for: ";

                List<int> affectedCrates = SolutionOpportunities[crateIndex]
                    .OrderBy(index => index)
                    .Select(index => index + 1)
                    .ToList();
            ...
            foreach (string push in SolutionPushSequence)
            {
                report += push + "\r\n";
            }
