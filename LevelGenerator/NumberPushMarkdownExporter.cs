...
namespace NovaWright.NumberPush.LevelGenerator
{
    public static class NumberPushMarkdownExporter
    {
            ...
            string outputDirectory)
        {
            ...
            string fileName =
                $"Level{level.LevelNumber:D3}-Solution.md";
            ...
            markdown.AppendLine("---");
            markdown.AppendLine(
                $"title: \"Number Push Level {level.LevelNumber}\"");
            ...
            markdown.AppendLine(
                $"# Number Push Level {level.LevelNumber}");
            markdown.AppendLine();
            ...
            markdown.AppendLine(
                $"- **Crates:** {level.Crates.Count}");
            markdown.AppendLine(
                $"- **Interior Walls:** " +
                $"{GetInteriorWallCount(level)}");
            markdown.AppendLine(
                $"- **Minimum Solution:** " +
                $"{solution.MinimumPushes} pushes");
            markdown.AppendLine(
                $"- **Generator Seed:** {seed}");
    ...
    $"Distance {crate.Distance}");
            ...
            {
                markdown.AppendLine(
                    "This level does not have a solution.");
                markdown.AppendLine();
                 ...
                 i++)
            {
                ...
                markdown.AppendLine(
                    $"- **Direction:** " +
                    $"{GetDirectionName(step.Direction)}");
                ...
                markdown.AppendLine(
                    $"- **Player Start:** " +
                    $"{FormatPoint(step.PlayerStart)}");

                markdown.AppendLine(
                    $"- **Player Push Position:** " +
                    $"{FormatPoint(step.PlayerPushPosition)}");

                markdown.AppendLine(
                    $"- **Crate Start:** " +
                    $"{FormatPoint(step.CrateStart)}");

                markdown.AppendLine(
                    $"- **Crate End:** " +
                    $"{FormatPoint(step.CrateEnd)}");

                markdown.AppendLine(
                    $"- **Player Walk:** " +
                    $"{FormatPath(step.PlayerPath)}");
        ...
        private static int GetInteriorWallCount(
            NumberPushLevel level)
        {
            int outerWallCount =
                (level.Columns * 2) +
