namespace NovaWrightNumberPush
{
    ...
    new Size(
        ...
        650);
            ...
            List<int> availableLevelNumbers =
                gameSession.GetAvailableLevelNumbers();

            progressRepository.UpdateAvailableLevels(
                progress,
            ...
            continueButton.ForeColor =
                Color.White;

            continueButton.BackColor =
                Color.FromArgb(24, 29, 41);

            continueButton.FlatStyle =
                FlatStyle.Flat;

            continueButton.FlatAppearance.BorderColor =
                Color.FromArgb(70, 170, 255);
    ...
    new CheckBox();
            ...
            levelListBox.BackColor =
                Color.FromArgb(24, 29, 41);

            levelListBox.ForeColor =
                Color.White;

            levelListBox.BorderStyle =
                BorderStyle.FixedSingle;

            levelListBox.Location =
                new Point(30, 170);

            levelListBox.Size =
                new Size(640, 340);

            levelListBox.SelectedIndexChanged +=
                LevelListBox_SelectedIndexChanged;
        ...
        private void LoadLevels()
        {
            ...
            foreach (int levelNumber in levelNumbers)
            {
                NumberPushLevelProgress? levelProgress =
                    progressRepository.GetLevelProgress(
                        progress,
                        levelNumber);
                ...
                else if (levelProgress?.Completed == true)
                {
                    if (levelProgress.BestPushes.HasValue)
                    {
                        status =
                            $"Completed — Best: " +
                            $"{levelProgress.BestPushes.Value} pushes";
                    }
                    else
                    {
                        status = "Completed";
